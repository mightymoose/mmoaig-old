module MmoaigAPI.MatchInstances (createMatchInstanceForMatch, loadMatchInstanceList) where

import Database.PostgreSQL.Simple (Connection)
import Control.Monad.IO.Class (liftIO)
import Database.Beam.Postgres (runBeamPostgres)
import Database.Beam (default_, runSelectReturningList, select, all_, limit_, guard_, (==.), val_, insertExpressions, orderBy_, asc_, currentTimestamp_)
